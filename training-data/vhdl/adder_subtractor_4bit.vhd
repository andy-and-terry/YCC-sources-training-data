library IEEE;
use IEEE.STD_LOGIC_1164.ALL;
use IEEE.NUMERIC_STD.ALL;

entity Adder_Subtractor_4bit is
    Port ( a, b     : in  STD_LOGIC_VECTOR(3 downto 0);
           sub      : in  STD_LOGIC;
           result   : out STD_LOGIC_VECTOR(3 downto 0);
           cout     : out STD_LOGIC;
           overflow : out STD_LOGIC);
end Adder_Subtractor_4bit;

architecture Behavioral of Adder_Subtractor_4bit is
    signal b_xor          : STD_LOGIC_VECTOR(3 downto 0);
    signal sum_ext        : UNSIGNED(4 downto 0);
    signal result_internal : STD_LOGIC_VECTOR(3 downto 0);
begin
    b_xor   <= b xor (others => sub);
    sum_ext <= ('0' & unsigned(a)) + unsigned('0' & b_xor) + sub;

    result_internal <= std_logic_vector(sum_ext(3 downto 0));
    result   <= result_internal;
    cout     <= sum_ext(4);
    overflow <= (a(3) = b_xor(3)) and (result_internal(3) /= a(3));
end Behavioral;
