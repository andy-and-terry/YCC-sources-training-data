       ...
       PROCEDURE DIVISION.
           ...
           PERFORM UNTIL WIDTH >= N
               ...
               PERFORM UNTIL LEFT-START > N
                   ...
                   COMPUTE R-IDX = LEFT-END + 1
                   MOVE LEFT-START TO T-IDX
                   PERFORM UNTIL L-IDX > LEFT-END OR R-IDX > RIGHT-END
                       IF ARR-ITEM(L-IDX) <= ARR-ITEM(R-IDX)
                           MOVE ARR-ITEM(L-IDX) TO TEMP-ITEM(T-IDX)
                           ADD 1 TO L-IDX
                       ELSE
                           MOVE ARR-ITEM(R-IDX) TO TEMP-ITEM(T-IDX)
                           ADD 1 TO R-IDX
                       END-IF
                       ADD 1 TO T-IDX
                   END-PERFORM
                   PERFORM UNTIL L-IDX > LEFT-END
                       MOVE ARR-ITEM(L-IDX) TO TEMP-ITEM(T-IDX)
                       ADD 1 TO L-IDX
                       ADD 1 TO T-IDX
                   END-PERFORM
