function binarySearch(a: readonly number[], target: number, lo: number, hi: number): number {
  while (lo <= hi) {
    const mid = (lo + hi) >>> 1;
...
export function exponentialSearch(a: readonly number[], target: number): number {
